...
function pickerDateRange(inputId, options) {
    ...
    //今天的按钮
    this.todayBtn = 'todayBtn_' + suffix;
    // 上一个月的按钮
    ...
    //记录输入框中开始日期和结束日期
    this.input_start = this.mOpts.startDate;
    this.input_end = this.mOpts.endDate;
                ...
                '<div class="ta_calendar_footer cf" ' + (this.mOpts.autoSubmit ? ' style="display:none" ' : '') + '>',
                    '<div class="frm_msg">',
                        '<div id="' + this.dateRangeDiv + '">',
                            '<input type="text" class="ta_ipt_text_s input_start" name="' + this.startDateId + '" id="' + this.startDateId + '" value="' + this.mOpts.startDate + '" />',
                            '<span class="' + this.mOpts.joinLineId + '">-</span>',
                            '<input type="text" class="ta_ipt_text_s input_end" name="' + this.endDateId + '" id="' + this.endDateId + '" value="' + this.mOpts.endDate + '" />',
                            '<input class="ta_ipt_text_s today_btn" type="button" id="' + this.todayBtn + '" value="今天"/><br />',
                        '</div>',
    ...
    //今天按钮点击事件
    $('#' + this.todayBtn).bind('click', function () {
        // body...
        var current = new Date();
        var month = current.getMonth()+1;
        var end = current.getFullYear()+'-'+month+'-'+current.getDate();
        // if(__method.dateInput.indexOf('endDate_')>=0){
        //     __method.dateInput = 'endDate_' + end;
        //     __method.endCompareDateId = __method.dateInput;
        // }else {
        //     __method.dateInput = 'startDate_' + end;
        //     __method.startDateId = __method.dateInput;
        // }
        __method.selectDate(end);
        $('.input_end').val(end);
        __method.markClick = false;
        __method.calendar_endDate.setMonth(new Date($('.input_start').val()).getMonth()+1, 1);
        __method.mOpts.endDate = __method.date2ymd(current).join('-');
        __method.init(0);
        __method.show(false, __method);
        __method.input_start = $('.input_start').val();
        __method.input_end = $('.input_end').val();

        // if(__method.markClick==false){
        //     __method.markClick = true;
        // }else {
    ...
    //日期开始输入框添加输入监听事件
    $('.input_start').bind('keypress',function () {
        ...
        if (event.keyCode == '13' && date != '') {
          ...
          if (!isNaN(timestamp2)) {
            var sDate = new Date($('.input_start').val());
            var eDate = new Date($('.input_end').val());
            var sTime = sDate.getTime();
            var eTime = eDate.getTime();
            var nowTime = new Date();
            if(sTime>nowTime){
                alert('开始日期不能晚于当前日期');
                $('.input_start').val(__method.input_start);
                __method.calendar_endDate.setMonth(new Date($('.input_start').val()).getMonth()+1, 1);
                var current = new Date($('.input_end').val());
                __method.mOpts.endDate = __method.date2ymd(current).join('-');
                __method.init(0);
                __method.show(false, __method);
                __method.input_start = $('.input_start').val();
            ...
            __method.calendar_endDate.setMonth(new Date(date).getMonth()+1, 1);
            var current = new Date(date);
            var month = current.getMonth()+2;
            var end = current.getFullYear()+'-'+month+'-'+current.getDate();
            __method.mOpts.endDate = __method.date2ymd(new Date(end)).join('-');
            __method.init(0);
            __method.show(false, __method);
            input_start = $('.input_start').val();
            input_end = $('.input_end').val();
            ...
            __method.input_start = $('.input_start').val();
            __method.input_end = $('.input_end').val();
            // $('.input_end').val(__method.input_end);
          }
        }
    })
    //日期结束输入框添加输入监听事件
    $('.input_end').bind('keypress',function () {
        var date = $(this).val().trim();
        if (event.keyCode == '13' && date != '') {
...
pickerDateRange.prototype.selectDate = function (ymd) {
    ...
    if (this.startDateId == this.dateInput) {
        ...
        if (-1 == this.compareStrDate(ymd, $('#' + this.startDateId).val())) {
            ...
            this.input_end = $('.input_end').val();
            // 更改对应输入框的值(开始时间)
            $('#' + this.startDateId).val(ymdFormat);
            this.input_start = $('.input_start').val();
...
pickerDateRange.prototype.close = function (btnSubmit) {
    ...
    if (btnSubmit) {
        ...
        if (this.mOpts.shortOpr === true) {
            //格式转换为YYYY年MM月d日
            var date = new Date($('#' + this.startDateId).val());
            var month = date.getMonth() + 1;
            var startDate = date.getFullYear() + '年' + month + '月' + date.getDate() + '日';
            
            $('#' + this.inputId).val(startDate);
            ...
            var month = date.getMonth() + 1;
            var startDate = date.getFullYear() + '年' + month + '月' + date.getDate() + '日';
            date = new Date($('#' + this.endDateId).val());
            month = date.getMonth() + 1;
            var endDate = date.getFullYear() + '年' + month + '月' + date.getDate() + '日';

            $('#' + this.inputId).val(startDate + ('' == endDate ? '' : this.mOpts.defaultText + endDate));
        }
        ...
        if (input && input.tagName == 'INPUT') {
            //格式转换为YYYY年MM月d日
            var temp = _val.split(this.mOpts.defaultText);
            ...
            month = date.getMonth() + 1;
            var endDate = date.getFullYear() + '年' + month + '月' + date.getDate() + '日';
            $('#' + this.inputId).html(startDate+this.mOpts.defaultText+endDate);
...
pickerDateRange.prototype.fillDate = function (year, month, index) {
    ...
    if (isTaTheme) {
       ...
       var cap = document.createElement('caption');
        ...
        if (!this.mOpts.magicSelect) {
            if (0 == index) {
                $(td).append('<a href="javascript:void(0);" id="' + this.nextMonth + '"><i class="i_next icon icon-calendar-right"></i></a>');
            }
            // 如果是第一个月的日期，则加上上一个月的链接
            if (index + 1 == this.mOpts.calendars) {
                $(td).append('<a href="javascript:void(0);" id="' + this.preMonth + '"><i class="i_pre icon icon-calendar-left"></i></a>');
            }
        }
    ...
    for (var d = dateBegin; d.getTime() <= dateEnd.getTime() ; d.setDate(d.getDate() + 1)) {
        ...
        td = document.createElement('td');
        td.innerHTML = '<span>'+d.getDate()+'</span>';
        if ('' != tdClass) {
